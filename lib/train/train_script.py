...
from lib.models.METrack import build_metrack
# forward propagation related
from lib.train.actors import ViPTActor
from lib.train.actors import METrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # coding
    elif settings.script_name == "METrack":
        net = build_metrack(cfg)
    else:
    ...
    if settings.script_name == "vipt":
        ...
        # coding
        focal_loss = FocalLoss()
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'cls': BCEWithLogitsLoss(), 'me': mse_loss}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1.0, 'cls': 1.0, 'me': 0.0001}
        actor = METrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
